...
from models.postprocessing import A2DSentencesPostProcess, ReferYoutubeVOSPostProcess, COCOPostProcess, PostProcess, \
    PostProcessSegm
...
class SOC(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.use_dab = config.DeformTransformer['use_dab']
        print('******* DAB :', self.use_dab, '*******')
        ...
        self.spatial_decoder = FPNSpatialDecoder(d_model, 2 * [d_model] + [self.backbone.num_channels[0]],
                                                 config.mask_kernels_dim)
        ...
        self.ca_text_frame = config.ca_text_frame
        print('******* FRAME-LEVEL :', self.ca_text_frame, '*******')
        self.ca_text_video = config.ca_text_video
        print('******* VIDEO-LEVEL :', self.ca_text_video, '*******')
    ...
    def forward(self, samples: NestedTensor, valid_indices, text_queries, targets):
        ...
        query_embeds = self.query_embed.weight  # position query [num_queries, C]
        tgt = torch.zeros_like(query_embeds)  # content query
        tgt = repeat(tgt, 'nq c -> b t nq c', b=B, t=T)
        if self.use_dab:
            query_embeds = self.refpoint_embed.weight  # [num_queries, 4]
        # text_embed = repeat(text_sentence_feature, 'b c -> b t q c', t=T, q=self.num_queries)

        #####################
        text_embed = langs[-1]
        text_embed = torch.mean(text_embed, dim=1, keepdim=True)
        text_embed_pos = rearrange(text_pos, 'l b c -> b l c')
        text_embed_pos = torch.mean(text_embed_pos, dim=1, keepdim=True)

        if self.ca_text_frame:  # * [FRAME-LEVEL] cross attention for text
            text_embed_f = text_embed.repeat(1, self.num_queries, 1)
            text_embed_pos_f = text_embed_pos.repeat(1, self.num_queries, 1)
        else:
            text_embed_f = None
            text_embed_pos_f = None

        if self.ca_text_video:  # * [VIDEO-LEVEL] cross attention for text
            text_embed_v = text_embed  # b l c
            text_embed_pos_v = text_embed_pos  # b l c
        else:
            text_embed_v = None
            text_embed_pos_v = None
        #####################
        ...
        hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, inter_samples = \
            self.transformer(srcs, tgt, masks, poses, query_embeds,
                             text_embed=text_embed_f, text_embed_pos=text_embed_pos_f,
                             text_word_masks=text_word_masks)
        ...
        voc_hs = self.voc(hs, text_sentence_feature,
                          text_embed=text_embed_v, text_embed_pos=text_embed_pos_v, text_word_masks=text_word_masks)
        # VOC_hs [L B NQ C]
        ...
        for lvl in range(hs_voc.shape[0]):
            dynamic_mask_head_params = self.controller(
                hs_voc[lvl])  # [batch_size*time, num_queries_per_frame, num_params]
...
def build(args):
    ...
    criterion = SetCriterion(matcher=matcher, weight_dict=weight_dict, eos_coef=args.eos_coef, text_refer=args.vl_loss,
