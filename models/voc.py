...
class VOC(nn.Module):
    ...
    def __init__(
        ...
        self.transformer_cross_attention_layers_t = nn.ModuleList()  #* cross attention for text
        ###------------------------------------------------------
        ...
        #* video-level object query initialization
        self.video_query = config['video_query']
        if self.video_query == 'embed':
        ...
        for _ in range(self.num_layers):
            ...
            self.transformer_cross_attention_layers_t.append(
                CrossAttentionLayer(
                    d_model=config['input_dim'],
                    nhead=config['nheads'],
                    dropout=0.1,
    ...
    def forward(self, frame_query, language_query,
        ...
        elif self.video_query == 'lang':
            language_query = language_query.unsqueeze(1).unsqueeze(1) #[B C]
            output = language_query.repeat(1, L, self.num_queries, 1) #[B, L, NQ, C]
            output = rearrange(output, 'b l nq c -> nq (l b) c') # cQ, LB, C
        elif self.video_query == 'zero':
            output = torch.zeros_like(query_embed) # cQ, LB, C

        #####################
        if text_embed is not None:
            text_embed = rearrange(text_embed, 'b i c -> i b c')
            text_embed = text_embed.unsqueeze(1)
            text_embed = text_embed.repeat(1, L, 1, 1)
            text_embed = rearrange(text_embed, 'i l b c -> i (l b) c')
            text_embed_pos = rearrange(text_embed_pos, 'b i c -> i b c')
            text_embed_pos = text_embed_pos.unsqueeze(1)
            text_embed_pos = text_embed_pos.repeat(1, L, 1, 1)
            text_embed_pos = rearrange(text_embed_pos, 'i l b c -> i (l b) c')
        #####################
        ...
        for i in range(self.num_layers):
            ...
            #* cross attention for text
            if text_embed is not None:
                output = self.transformer_cross_attention_layers_t[i](
                    output, text_embed,
                    memory_mask=None,
                    memory_key_padding_mask=None,
                    pos=text_embed_pos, query_pos=query_embed
                )

            # cross attention for visual
