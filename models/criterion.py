...
class SetCriterion(nn.Module):
    ...
    def vl_loss(self, output, targets, indices, frame_size, **kwargs):
        ...
        query_text_sim = output_logit @ text_sentence_feature.transpose(1, 2) #[B Nq 1] #* 쿼리와 텍스트 간 유사도 계산 (내적)
        ...
        #* 참조되는 개체에 대한 인덱스에 해당하는 위치에 1.0 값을 설정함
        gt[torch.arange(bs), query_referred_indices] = torch.tensor([1.0], device=device, dtype=gt.dtype)
        # matcher loss #* 손실 값을 계산하는 부분 : Negative Log Likelihood 손실을 사용함
        query_text_sim = query_text_sim.log_softmax(dim=-1) #* 모델의 logit 값(유사도값)을 로그 확률 값으로 변환
        loss = gt*query_text_sim #[t*b Nq] #* GT와 로그 확률 값(유사도값)을 곱해 손실 계산
        eos_coef = torch.full(loss.shape, self.eos_coef, device=loss.device) #* eos_coef: 0.1
        eos_coef[torch.arange(bs), query_referred_indices] = 1.0
        loss = loss * eos_coef

        loss = loss.mean(0).sum()
        losses = {'loss_con': -loss}  #* 손실을 계산하고, 음수를 취해 최적화 목적으로 손실을 최소화하도록 만듦
        return losses
